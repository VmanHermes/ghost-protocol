...
#[tokio::test]
async fn test_non_mcp_agent_receives_ghost_env_and_preamble() {
    with_daemon(|base_url| async move {
        let client = reqwest::Client::new();

        // Verify the tool proxy is accessible (this is what non-MCP agents use)
        let resp = client
        ...
        assert!(
            result.contains("machine") || result.contains("host") || result.contains("Ghost"),
            "check_mesh should return mesh context, got: {result}"
        );

        // Verify report_outcome works through the proxy
        let resp = client
            .post(format!("{base_url}/api/ghost/tools/report_outcome"))
            .json(&json!({
                "category": "test",
        ...
        assert_eq!(resp.status(), 200);
        let body: serde_json::Value = resp.json().await.unwrap();
        assert!(body["result"].as_str().unwrap().contains("Outcome recorded"));

        // Verify recall works (may return empty results, that's fine)
        let resp = client
            .post(format!("{base_url}/api/ghost/tools/recall"))
